...
\addcontentsline{toc}{section}{Introduction}
		...
		Nous avons rapidement trouvé des travaux de recherche de l'Inria ainsi qu'un tutoriel qui s'intéressaient à ce sujet \cite{ref1}. L'idée qui nous a tout de suite plu était de pouvoir reconstruire des images bruitées, partiellement détruites ou marquées. Sur la figure \ref{mot}, nous montrons les résultats des recherches de l'Inria \cite{ref1} qui nous ont motivés.
...
\begin{figure}[!h]
	...
	\label{mot}
\end{figure}
		La technique sous jacente, appelée sparse coding (ou encore "encodage creux"), consiste à apprendre un dictionnaire à partir de sous images ( appelés "patches") extraites de l'image initiale. Le dictionnaire est appris de telle façon que chaque patch peut s'exprimer relativement bien comme une combinaison linéaire d'un petit nombre (dout l'adjectif "sparse") d'éléments du dictionnaire. On peut ensuite utiliser le dictionnaire pour reconstruire les patches, même s'ils sont bruités. La figure \ref{dic} montre un exemple de dictionnaire obtenu par l'Inria \cite{ref1}. On peut y apercevoir des "features" de l'image.
...
\end{algorithm}
	...
	\subsection{Reconstruction d'image}
		Pour reconstruire l'image, nous utilisons l'agorithme d'orthogonal matching pursuit (OPM) \cite{ref2}. La méthode proposée consiste à minimiser l'énergie une fois le dictionnaire fixé. Pour $k >0$ fixé, on s'intéresse pour chaque patch $\vec{x} \in \mathbb{R}^{m}$ (précédemment centré) à : $$\min_{\alpha \in \mathbb{R}^{k}} \parallel \vec{x} - \vec{D}\alpha\parallel_{2}^{2} S.C. \parallel\alpha\parallel_{0} < k$$
...
\subsection{Python}
		...
		Il a été fait usage des packages numpy (pour des opérations plus efficaces sur les tableaux), scipy et PIL (pour la conversion image/tableau) et scikit-learn (pour l'optimisation LARS lasso et OMP). Notre code python est trouvable dans le répertoire /python de notre dépôt git. Les packages nécessaires pour le faire tourner sont inclus dans beaucoup de distributions python scientifiques (anaconda pour ne citer qu'elle).
		...
		\begin{figure}[!h]
			...
			\end{center}
			\caption{Résultats de l'implementation python (image originale, reconstruite et dictionnaire). Temps d'exécution : environ 3 min.}
		\end{figure}
		...
		Le maximum que nous ayons fait a donné le résultat ci dessous, relativement probant (image de $1500\times 1000$ pixels, l'algorithme a tourné pendant 10-20 minutes (apprentissage + reconstitution)) :\\
		...
		\begin{figure}[!h]
			...
			\end{center}
			\caption{Image reconstruite à partir de l'image bruitée (bruit uniforme)}
		\end{figure}
		...
		\begin{figure}[!h]
			...
			\end{center}
			\caption{Dictionnaire appris sur l'image non bruitée}
		\end{figure}
		\begin{figure}[!h]
			\begin{center}
...
\addcontentsline{toc}{section}{Conclusion}
		...
		Ce projet était une bonne occasion d'explorer l'application de domaines mathématiques (optimisation convexe, statistiques notamment) liés à l'apprentissage statistique au traitement d'images. Les résultats semblent confirmer le potentiel de la technique de sparse coding, même si les performances de notre implémentation laissent à désirer. En effet, la grande force de cette technique est que l'on obtient un résultat similaire que le dictionnaire soit appris sur l'image d'origine ou l'image bruitée! On peut donc débruiter une image sans l'image d'origine.
